...
class {nome}Controller extends \core\Controller
{
    ...
    {
        ${modelvar} = $this->model('{modelName}');
        {sets}
        if(${modelvar}->save() === true) {
   ...
   public function editarcampo(): void
	{
		...
		${modelvar} = $this->model('{modelName}');
		if (${modelvar}->updateFields($_POST, $id)) {
