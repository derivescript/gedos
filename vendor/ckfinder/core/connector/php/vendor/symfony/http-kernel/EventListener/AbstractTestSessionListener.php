...
abstract class AbstractTestSessionListener implements EventSubscriberInterface
{
    ...
    public function onKernelResponse(ResponseEvent $event)
    {
        ...
        if ($session instanceof Session ? !$session->isEmpty() || (null !== $this->sessionId && $session->get_id() !== $this->sessionId) : $wasStarted) {
            ...
            $event->getResponse()->headers->setCookie(new Cookie($session->getName(), $session->get_id(), 0 === $params['lifetime'] ? 0 : time() + $params['lifetime'], $params['path'], $params['domain'], $params['secure'], $params['httponly'], false, $params['samesite'] ?: null));
            $this->sessionId = $session->get_id();
        }
    }
